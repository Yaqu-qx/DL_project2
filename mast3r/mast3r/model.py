...
class AsymmetricMASt3R(AsymmetricCroCo3DStereo):
    ...
    def set_downstream_head(self, output_mode, head_type, landscape_only, depth_mode, conf_mode, patch_size, img_size, **kw):
        assert img_size[0] % patch_size == 0 and img_size[1] % patch_size == 0, f'img_size={img_size} must be multiple of patch_size={patch_size}'
...
class AsymmetricMASt3RWithDUNEBackbone(torch.nn.Module):
    ...
    def _encode_image(self, image, true_shape):
        ...
        if self.landscape_only:
            assert W >= H, f'image should be in landscape mode, but got W={W} H={H}'
